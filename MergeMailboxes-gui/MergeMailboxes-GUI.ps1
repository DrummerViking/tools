...
 
    ...
    Write-Host "$((Get-Date).ToString("MM-dd-yyyy HH:mm:ss")) - Collecting Soft-Deleted mailboxes..."
    ...
    ($sourceMailbox.EmailAddresses).remove($address2)
        ...
        if ($checkboxSourceisArchive.Checked) {
            $SourceGuid = $sourceMailbox.ArchiveGuid ; $sourceSwitch = $True
        }
        else { $SourceGuid = $sourceMailbox.ExchangeGuid ; $sourceSwitch = $False }

        if ($checkboxTargetisArchive.Checked) {
            $targetGuid = $targetMailbox.ArchiveGuid ; $targetSwitch = $True
        }
...
Add these addresses to the target user account. Be aware that if the user is Synchronized from Active Directory, you need to do it on-premises."
    ...
    #
    $LabelListSoftDeletedMailboxes.Location = New-Object System.Drawing.Point(5, 120)
    $LabelListSoftDeletedMailboxes.Size = New-Object System.Drawing.Size(180, 30)
    ...
    $LabelSearchTargetMailbox.Size = New-Object System.Drawing.Size(130, 30)
    $LabelSearchTargetMailbox.Name = "LabelSearchTargetMailbox"
    $LabelSearchTargetMailbox.Text = "Search Target Mailbox:"
    ...
    $TxtBoxSearchTargetMailbox.Location = New-Object System.Drawing.Point(630, 118)
